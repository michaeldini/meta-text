/**
 * @fileoverview Index file for SourceDocPage components
 * 
 * Centralized exports for all source document page components including
 * the list view and detail view pages.
 * 
 * @author MetaText Development Team
 * @version 1.0.0
 * @since 2025-07-09
 */

// Export the source document list page component
export { default as SourceDocListPage } from './SourceDocListPage';

// Export the source document detail page component  
export { default as SourceDocDetailPage } from './SourceDocDetailPage';

// Export custom hooks
export { useSourceDocDetailData } from './useSourceDocDetailData';
